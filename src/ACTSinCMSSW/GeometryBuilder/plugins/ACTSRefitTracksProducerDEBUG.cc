    ...
    for(auto trk_state : reFittedTrack.trackStatesReversed()){
        ...
        if(trk_state.hasReferenceSurface()){
            ...
            if(this_surf.associatedDetectorElement() != nullptr){
                ...
                auto this_t = this_surf.transform(gCtx);
                Acts::GeometryView3D::drawSurface(Surf_obj, this_surf, gCtx, this_t, Acts::ViewConfig{});
            } 
            // else {
            ...
            if(trk_state.hasCalibrated()){ 
                ...
                // Get the local hit and the direction
                // NEW METHOD 
                auto params = trk_state.parameters();
                auto covariance = trk_state.covariance();
                double x_cluster = params[Acts::eBoundLoc0];
                double x_err = std::sqrt(covariance(Acts::eBoundLoc0,Acts::eBoundLoc0));
                double y_cluster = params[Acts::eBoundLoc1];
                double y_err = std::sqrt(covariance(Acts::eBoundLoc1,Acts::eBoundLoc1));
                ...
                // std::cout << "[Hits from final track state] X = " << x_cluster << " +- " << x_err << "; Y = " << y_cluster << " +- " << y_err << std::endl;
                // std::cout << "[Direction from final track state] " << direction.transpose() << std::endl;
                
                // Convert it into a global hit
                auto globalPos =  this_surf.localToGlobal(gCtx, Acts::Vector2(x_cluster, y_cluster), direction);
...
reco::Track::CovarianceMatrix convertCovACTStoCMSSW(const std::vector<double> actsParams, 
                                                    const std::shared_ptr<const Acts::Surface> pSurface, 
                                                    const Covariance cov_acts, 
                                                    const Acts::Vector3 fitted_dir){

    /// NOTE: you can find the definition of CMSSW covariance matrix here:
    // https://github.com/cms-sw/cmssw/blob/aa0252b8b8377cd5ab3075fa85a366337680612c/DataFormats/TrackReco/interface/TrackBase.h#L17

    // ===============================
    // Step 1: Copy ACTS covariance (get rid of time)
    // ===============================
    ...
    // Convert mm -> cm for coordinates and spatial cross-terms
    // double scale = 0.1; // mm -> cm
    // for(int i : {Acts::eBoundLoc0, Acts::eBoundLoc1}) {
    //     for(int j = 0; j < 5; ++j){
    //         Cacts(i,j) *= scale;
    //         Cacts(j,i) *= scale;
    //     }
    ...
    // ===============================
    // Step 2: Extract ACTS parameters (loc0, loc1, phi, theta, qOverP)
    // ===============================
    ...
    // Acts::Vector3 fitted_pos = pSurface->localToGlobal(Acts::GeometryContext{}, Acts::Vector2(l0, l1), fitted_dir);
    // double x = fitted_pos[0] / 10;
    // double y = fitted_pos[1] / 10;
    // double z = fitted_pos[2] / 10;

    // double x = l0 * u.x() + l1 * v.x();
    // double y = l0 * u.y() + l1 * v.y();
    // double z = l0 * u.z() + l1 * v.z();

    // ===============================
    // Step 4: Build the 5x5 Jacobian
    // ===============================
    ...
    // ===============================
    // Step 6: Copy the CMSSW covariance
    // ===============================
    ...
    // ===============================
    for (int i = 0; i < 5; ++i){
        if (cov_cmssw(i,i) <= 0.0){
            std::cerr << "Warning: variance of parameter " << i << " <= 0!" << std::endl;
        }
    }

    // UNCOMMENT IF YOU WANT TO NOT CHANGE ANYTHING:
    // for (int i = 0; i < 5; ++i){
    //     for (int j = 0; j <= i; ++j){
    //         cov_cmssw(i,j) = cov_acts(i,j);
    //     }
    // }
...
//     // Apply the Jacobian:
//     Eigen::Matrix<double,5,5> Ccms = J * Cacts * J.transpose();

//     // Convert the covariance matrix back:
//     reco::Trak::CovarianceMatrix cov_cmssw;
//     for (int i = 0; i < 5; ++i) {
//     for (int j = 0; j <= i; ++j) {
//         cov_cmssw(i,j) = Ccms(i,j);
//     }
//     }
...
void ACTSRefitTracksProducerDEBUG::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    // ===== DEBUG: MAP THE B FIELD =====
    // Sample the field using "cylindrical coordinates"
    // std::ofstream outF_B("BfieldMap.txt");
    // Acts::MagneticFieldProvider::Cache cache;

    // for(double r = 0; r < 1400 ; r += 14){          // 100 bins
    //     for(double z = -3000; z < 3000; z += 60){  
    //         std::cout << "Computing B at " << r << " 0 " << z << " " << std::endl;
    //         auto B_res = magFieldPtr->getField(Acts::Vector3{r,0,z}, cache);
    //         if(B_res.ok()){
    //             outF_B << r << " 0 " << z << " " << B_res.value().transpose()[2] / Acts::UnitConstants::T << '\n';
    //         }
    //     }
    // }
    // outF_B.close();
